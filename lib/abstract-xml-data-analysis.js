...
function AbstractXMLDataAnalysis(xml) {
  if (xml) {
...
AbstractXMLDataAnalysis.prototype.analyze = function (nodePath, config) {
  if (config) {
    if (config.logLevel) {
  ...
  if (nodes.length == 0) {
    ...
    if (this.features == "datatypes") {
      for (var i = 0; i < nodes.length; i++) {
        var node = nodes[i];
        for (var k = 0; k < p_a.length; k++) {
          var path = p_a[k];
          ...
          if (path.indexOf('@') != -1) {
            ...
            utilities.analyseValue(this.analysis, path, value);
    ...
    } else if (this.features == "all" || this.features == "keys") {
          ...
          if (!(p.buildOn && p_c[p.buildOn].valid) || p.paths.length == singleKeyPathsCount) {
            ...
            for (var k = 0; k < paths.length; k++) {
              ...
              if (path.indexOf('@') != -1) {
                ...
                if (this.features == "all" && paths.length == singleKeyPathsCount) {
                  utilities.analyseValue(this.analysis, path, value);
                }
              } else {
                selectResult = xpath.select(path, node);
    ...
    if (this.features == "all" || this.features == "structure") {
      returnValues.structure = structure;
    }

    if (this.features == "all" || this.features == "keys") {
      returnValues.keys = p_a;
    }

    if (this.features == "all" || this.features == "datatypes") {
      returnValues.analysis = this.analysis;
    }
...
AbstractXMLDataAnalysis.prototype.atLeastOneNodeInAllGroups = function (groups, index) {
...
module.exports = {
  AbstractXMLDataAnalysis: AbstractXMLDataAnalysis
};