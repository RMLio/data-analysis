...
var XMLSinglePassKeyDiscovery = function (xml) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        for (var j = 0; j < p_a.length; j++) {
          ...
          for (var k = 0; k < paths.length; k++) {
            ...
            if (path.indexOf('@') != -1) {
              var selectResult = xpath.select(path, node)[0];
              ...
              selectResult = xpath.select(path, node)[0];

              if (selectResult) {
                value = selectResult.firstChild.nodeValue;
              } else {
                value = "@@UNDEFINED@@";
              }
            }
            ...
            if (tree) {
              foundNodes = tree.search(value);
              //console.log('foundNodes = ' + foundNodes);
              var t = foundNodes.indexOf(i);
              if (t == -1) {
                tree.insert(value, i);
              } else {
                winston.silly('t = ' + t);
                ...
                for (var l = 0; l < foundNodes.length; l++) {
                  if (t != l) {
                    winston.silly('push');
                    temp.push(foundNodes[l]);
                  }
                }
          ...
          if (atLeastOneNodeInAllGroups(foundNodesGroup)) {
            p_a.splice(j, 1);
            j --;
          }
  ...
  function atLeastOneNodeInAllGroups(groups) {
