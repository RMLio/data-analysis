...
function analyseValue(analysis, path, value) {
  winston.debug("analysis: path = " + path + ", value = " + value);
  if ("@@UNDEFINED@@" === value) {
    value = null;
  }
  ...
  if (type == 1) {
    if (analysis[path].averageWords == undefined) {
    ...
    if (analysis[path].max == undefined) {
...
function getPaths(nodes, multiLevel, withPriority) {
  var possiblePaths = [];
...
function getStructure(paths, rockerVersion) {
